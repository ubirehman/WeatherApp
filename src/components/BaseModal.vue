            ...
            <div v-show="modalActive" class="absolute w-full bg-black bg-opacity-30 h-screen top-0 left-0 flex justify-center px-8">
                <Transition name="modal-inner">
                    <div v-if="modalActive"  class="p-4 bg-white self-start mt-32 max-w-screen-md">
                        <slot />
                        <button @click="$emit('close-modal')" class="text-white mt-8 bg-weather-secondary py-2 px-6">
                            Close
                        </button>
                    </div>
                </Transition>
...
<style scoped>
 ...
 {
    transition: all 0.3s cubic-bezier(.52, .02, .19, 1.02) .15s;
 }

 .modal-inner-leave-active
 {
    transition: all 0.3s cubic-bezier(.52, .02, .19, 1.02);
 }
